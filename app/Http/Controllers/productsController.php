...
class productsController extends Controller
{
    ...
    function postAdd(Request $request){
    	...
    	$products->quantity = (int)($request->quantity ?? 0);
    	...
    	if($request->hasFile('image')){
            ...
            */
            ...
            }while (file_exists(public_path("upload/image/$name_random")));

            $flag = $file->move(public_path("upload/image"), $name_random);
		...
		if($request->hasFile('imagesDetail')){
			foreach ($request->file('imagesDetail') as $file) {
				...
				if(isset($file)){
					...
					}while(file_exists(public_path('upload/detail/$name_img_rd')));
					$product_img->image = $name_img_rd;
					$product_img->id_product = $product_id;	
					$file->move(public_path('upload/detail'),$name_img_rd);
					$product_img->save();
				}
			} 
		}
